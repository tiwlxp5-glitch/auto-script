...
export function highlightBannedWords`
  ...
  code = code.replace(
    /export function highlightBannedWords\(text, foundWarnings\) \{[\s\S]*?if \(!text \|\| foundWarnings\.length === 0\) return text;\s*let highlightedText = text;/m,
  ...
  console.log('bannedWords.js secured!');
} else {
