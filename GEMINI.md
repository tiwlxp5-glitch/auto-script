...
6. **Launch Readiness & Auth Polish** (Current Session):
   ...
   - **History UX Polish**: Translated `Pro_MultiVersion` mode to Thai, made the History mode filter dynamic to support all new AI modes, added version badges (`ครั้งที่ X`) and precise timestamps to disambiguate duplicate script generations of the same product, and updated the TXT export filenames to append `_vX`.
